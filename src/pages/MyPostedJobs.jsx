...
const MyPostedJobs = () => {
  ...
  const fetchingJobsByEmail = async () => {
    axios
      .get(`${import.meta.env.VITE_API_URL}/jobs/${user?.email}`)
      .then((res) => setSpecificJobs(res.data));
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const allPostedJobsAfterDelete = [...specificJobs].filter(
            (job) => job._id !== jobId
          );
          setSpecificJobs(allPostedJobsAfterDelete);
          Swal.fire({
            title: "Deleted!",
